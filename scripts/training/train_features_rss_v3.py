# train_features_rss_v3.py
# Build train_features_rss_v3.csv using Google News RSS cache (retrieval_cache_rss.json)
# This is an OFFLINE feature builder (no paid APIs needed).
#
# Inputs:
#   --data  fake_news_simplified_LANGFILTER_labels01.csv
#   --cache retrieval_cache_rss.json
# Output:
#   train_features_rss_v3.csv  (default)
#
# Features:
#   num_evidence, max_rel, mean_rel, p90_rel,
#   max_ent, mean_ent,
#   max_con_used, mean_con,
#   body_success_rate
#
# Evidence text used for NLI:
#   Prefer extracted article body via trafilatura if available, else title+description.
...
def load_models():
    ...
    return device, emb, nli_tok, nli_model, nllb_tok, nllb_model

@torch.inference_mode()
def nli_probs(device, tok, model, premise: str, hypothesis: str) -> Tuple[float, float, float]:
...
# ---------------- feature compute ----------------
def compute_features(device, emb, nli_tok, nli_model, claim_en: str, evidence: List[Dict], top_k: int, sleep: float) -> Dict[str, float]:
    if not evidence:
        return {
            "num_evidence": 0.0,
    ...
    # Fixed contradiction gating (same idea as app)
    ...
    for it in top:
        ...
        ent, con, neu = nli_probs(device, nli_tok, nli_model, premise, claim_en)
    ...
    return {
        ...
        "mean_con": float(np.mean(con_useds)) if con_useds else 0.0,
...
def main():
    ...
    device, emb, nli_tok, nli_model, nllb_tok, nllb_model = load_models()
    ...
    for i, r in df_s.iterrows():
        ...
        feats = compute_features(device, emb, nli_tok, nli_model, claim_en, evidence, top_k=args.top_k, sleep=args.sleep)
