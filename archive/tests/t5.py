...
# ==============================
st.set_page_config(
    page_title="EL MASTER | Multimodal Truth Engine", 
    layout="wide",
...
APP_VERSION = "EL-MASTER-FINAL-UI-V2"
...
def show_landing_page():
    ...
    with c2:
        st.markdown(
            ...
            <div style="text-align: center;">
                <h1 style="font-size: 5rem; color: #58a6ff; margin-bottom: 0;">EL MASTER</h1>
                <p style="font-size: 1.5rem; letter-spacing: 2px; color: #8b949e; text-transform: uppercase;">
                    Multimodal Fact Verification Engine
                </p>
            </div>
            """, 
            unsafe_allow_html=True
    ...
    st.markdown("<h3 style='text-align: center; margin-bottom: 30px;'>TEAM OVERWATCH</h3>", unsafe_allow_html=True)
    
    col1, col2, col3, col4 = st.columns(4)
    
    team_data = [
        ("Project Lead", "Alice Smith", "Architecture & Vision"),
        ("AI Engineer", "Bob Jones", "Model Integration"),
        ("Backend Dev", "Charlie Day", "API & Systems"),
        ("Frontend", "Dana White", "UI/UX Design")
    ]
    
    cols = [col1, col2, col3, col4]
    
    for i, (role, name, desc) in enumerate(team_data):
        with cols[i]:
            st.markdown(
                f"""
                <div class="team-member">
                    <div style="font-size: 1.5rem; margin-bottom: 10px; color: #8b949e;">Member</div>
                    <div class="team-role">{role}</div>
                    <div style="font-size: 1.2rem; font-weight: bold; color: #f0f6fc;">{name}</div>
                    <div style="font-size: 0.9rem; color: #8b949e;">{desc}</div>
                </div>
                """,
                unsafe_allow_html=True
            )
...
def show_detector_page():
    st.title("Multimodal Truth Engine")
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("### About")
        st.info(
            "EL MASTER uses a hybrid Neuro-Symbolic architecture combining "
            "LLM reasoning with traditional Deep Learning classifiers."
        )
