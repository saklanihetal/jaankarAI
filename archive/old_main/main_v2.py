# app_master_v13_update_aware.py
# MASTER VERSION (v13) - "Update-Aware Logic"
#
# ADDRESSES USER FEEDBACK:
# 1. "Title is gimmicky": We revert to using "Title + Description" for NLI.
# 2. "Why Contradicted?": We fix the "1 vs 3 dead" issue using "Casualty Update Logic".
#
# NEW FEATURE (Restored from test3.py):
# - check_casualty_update(): Detects if a contradiction is just a death toll increase.
#   If (Evidence_Death_Count > Claim_Death_Count), it overrides CONTRADICTED -> SUPPORTED.
...
# CONFIG & STYLES
# ===========================
st.set_page_config(page_title="AI FactCheck Master (v13)", layout="wide")
...
# We remove sensational words to help NLI focus on facts
...
def neutralize_claim(text: str) -> str:
    """Removes sensational adjectives that confuse NLI models (e.g., Massive vs Cylinder)."""
    text = text.lower()
    for word in SENSATIONAL_ADJECTIVES:
        text = re.sub(r"\b" + word + r"\b", "", text)
    return normalize_text(text)

# --- NEW: UPDATE CHECKER (Restored from test3.py) ---
def check_casualty_update(claim_text: str, evidence_text: str) -> bool:
    """
    Returns True if evidence seems to be an UPDATE of the claim (e.g., death toll rose).
    Logic: Both mention death/injury, and Evidence_Num > Claim_Num.
    """
    ...
    # Must be about death
    if not (c_toks & death_keywords) or not (e_toks & death_keywords):
        return False
        
    # Extract numbers (simple heuristic)
    def get_max_num(txt):
    ...
    # If evidence number is higher, it's likely an update (1 dead -> 3 dead)
    # We use a sanity cap (e.g. < 100) to avoid comparing years (2024 vs 1)
    if c_num > 0 and e_num > c_num and e_num < 100:
        return True
...
# ===========================
def render_system():
    st.markdown("# 🕵️ AI FactCheck Master (v13)")
    st.markdown("Multi-source verification with Entity Lexicon, Update-Aware Logic, and Deepfake analysis.")
    ...
    if run_btn and txt_input:
        ...
        for it in top_items:
            ...
            if it.f_ent > it.f_con: 
                ...
                # *** CRITICAL FIX: CHECK FOR UPDATE ***
                # If NLI says Contradict, check if it's just a number update (1 -> 3 dead)
                if check_casualty_update(t_en_clean, premise):
                    it.nli_label = "ENTAILS (Update)"
                    it.f_ent = 0.95 # Override score
                    it.f_con = 0.05
                    it.is_update = True
        ...
        with col2:
            ...
            for it in top_items:
                ...
                # Highlight if Update Logic triggered
                status_badge = f"{it.nli_label}"
                if it.is_update:
                    status_badge += " 📈 (Death Toll Updated)"
