# app_master_v11_india_lexicon.py
# MASTER VERSION (v11) - "India-Aware" Logic
#
# INTEGRATION MAP:
# 1. CORE: CPU-Optimized Google Translate (from ui8)
# 2. LOGIC: India Entity Lexicon + Alias Swapping (from test2.py) -> CRITICAL FIX
# 3. CONTEXT: Consensus Terms + Locations (from test3.py/ui2) -> RESTORED
# 4. UI: Dual-Language Evidence (from ui4) -> RESTORED
# 5. PERF: Parallel API Fetching (from ui6) -> RESTORED
#
# LOGIC FIX:
# - Queries like "Massive explosion" are now forced to include entities ("Mysore Palace")
# - Low relevance (< 0.15) forces UNVERIFIED verdict.
...
# CONFIG & STYLES
# ===========================
st.set_page_config(page_title="AI FactCheck Master (India-Aware)", layout="wide")
...
<style>
    ...
    .term-tag { display: inline-block; padding: 4px 10px; border-radius: 4px; background: rgba(0, 188, 212, 0.15); border: 1px solid rgba(0, 188, 212, 0.3); font-size: 0.85rem; margin: 4px; }
</style>
""", unsafe_allow_html=True)

# --- INDIA LEXICON (FROM TEST2.PY) ---
INDIA_ENTITIES_SEED = [
...
def translate_to_english(text: str) -> str:
    """CPU-optimized translation using Google API"""
    if not text: return ""
    if GoogleTranslator:
...
def apply_india_aliases(text: str) -> List[str]:
    """Generates variants of a query using known aliases (Mysuru <-> Mysore)."""
    outs = [text]
...
def build_lexicon_queries(claim_en: str, nlp) -> List[str]:
    """Robust Query Builder from test2.py + ui7 logic."""
    claim_en = normalize_text(claim_en)
    
    # 1. Identify Entities (SpaCy + Lexicon)
    entities = []
    if nlp:
        doc = nlp(claim_en)
        entities = [e.text for e in doc.ents if e.label_ in {"PERSON", "ORG", "GPE", "LOC", "FAC"}]
    
    # Check against India Seed List (Regex match)
    ...
    keywords = [t for t in tokens if t in {"dead", "killed", "injured", "blast", "explosion", "retire", "arrested", "won", "lost", "ban", "released"}]
    
    queries = []
    
    # Strategy 1: Entity + Keyword (High Precision)
    if entities and keywords:
        queries.append(f"{entities[0]} {keywords[0]}")
    
    # Strategy 2: Just Entities
    if entities:
        queries.append(" ".join(entities[:3]))
        
    # Strategy 3: Full Claim
    queries.append(re.sub(r"[^\w\s]", "", claim_en))
    
    # 4. EXPAND ALIASES (Mysore -> Mysuru)
    final_queries = []
    for q in queries:
...
def fetch_evidence_parallel(queries: List[str]) -> List[EvidenceItem]:
    """Parallel fetch (Restored from ui6)."""
    items = []
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for q in queries:
            tasks.append(executor.submit(fetch_rss, q))
...
def get_common_terms(items: List[EvidenceItem]) -> List[Tuple[str, int]]:
    """Extract consensus terms (Restored from test3.py)."""
...
# ===========================
def render_system():
    st.markdown("# 🕵️ AI FactCheck Master (India-Aware)")
    st.markdown("Multi-source verification with Entity Lexicon, Deepfake, and NLI analysis.")
    ...
    if run_btn and txt_input:
        ...
        # 2. Queries (Lexicon Enhanced)
        status.write("🧠 Generating smart queries (India Lexicon)...")
        queries = build_lexicon_queries(t_en, nlp)
        st.code("\n".join(queries[:3]), language="text")
        
        # 3. Fetch (Parallel)
        status.write(f"🌐 Searching across sources ({len(queries)} queries)...")
        ...
        # 1. Verdict
        decision = "UNVERIFIED"
        # Adjusted logic: Low relevance means UNVERIFIED
        if not items or items[0].relevance < 0.15:
            decision = "UNVERIFIED"
        elif max_con > 0.4 and max_con > max_ent:
        ...
        with col2:
            ...
            st.markdown(f"**Common Terms:** {term_html}", unsafe_allow_html=True)
            
            # 3. Evidence List (Dual Language)
            ...
            for it in top_items:
                ...
                <div class="evidence-item">
                    ...
                    <div style="font-size:0.85em; opacity:0.8; margin-top:4px;">
                        {it.source} • Rel: {it.relevance:.2f} • {it.nli_label} (E:{it.f_ent:.2f} C:{it.f_con:.2f})
                    </div>
