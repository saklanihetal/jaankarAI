...
# Final integrated Streamlit app (patched):
# - Adds "Likely True" in UNVERIFIED case (language-only TF-IDF model uses p_real & p_fake)
...
def load_gate(path: str) -> object:
    ...
    # Sometimes joblib contains wrapper dict. Handle both.
...
def gate_predict_proba_safe(gate_model, feat_dict: Dict[str, float], feat_list: List[str]) -> Tuple[float, str]:
    """
    Returns: (p_pos, note)
    Uses DataFrame with named columns to reduce ordering bugs.
    If the gate throws feature-count error, returns 0 with note.
    """
    try:
...
with st.sidebar.expander("Run command", expanded=False):
    st.code("streamlit run app_streamlit_final_v2.py", language="bash")
...
if news_text and len(news_text.strip()) > 0:
    ...
    # Language-only: show both likely true and likely false
    p_real = None
    p_fake = None
    language_hint = "n/a"
    if tfidf_model is not None:
        try:
            ...
            # class 0 vs 1 meanings depend on your invert_label choice during training.
            # In our trainer, with --invert_label, class 1 becomes FAKE-style risk.
            p_real = float(pr[0])
            p_fake = float(pr[1])
    ...
    # If gates fail (still 0 due to mismatch), fallback to rules so you don't get UNVERIFIED with ent=0.98
    # These fallback rules are conservative.
    ...
    # If still UNVERIFIED, append likely true/false hint from language patterns
    st.subheader("Result")
    ...
    with c5:
        if p_fake is None:
            st.metric("Language hint", "n/a")
        else:
            st.metric("Language hint", language_hint)
    ...
    # Only show language hint if UNVERIFIED
    ...
    # Context mismatch explanation (generic, not hardcoded to replica)
