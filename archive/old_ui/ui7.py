# app_streamlit_final_v5_multipage_best.py
# V5 (Best): Multipage (Landing + System) + polished, consistent UI + strong API query generation
# - Landing page: project title + team details aligned to UI
# - System page: full pipeline (RSS + APIs + NLI + gates + CLIP + Deepfake + Rekognition)
# - Verdict appears in the blank space under Claim
# - No yellow warning blocks in main UI (neutral styled callouts instead)
# - Bigger title + slightly bigger Evidence text
# - APIs: robust keyword/entity query generation (Virat Kohli / retire / ODI style queries)
...
.verdict .k { font-size: 0.84rem; opacity: 0.75; margin-bottom: 0.25rem; }
.verdict .v { font-size: 1.55rem; font-weight: 800; letter-spacing: 0.3px; margin: 0; }
...
/* Evidence items (slightly bigger) */
.evidence {
  border: 1px solid rgba(255,255,255,0.08);
...
.evidence .title { font-weight: 700; font-size: 1.05rem; margin-bottom: 0.35rem; }
.evidence .meta { opacity: 0.78; font-size: 0.92rem; }
.evidence .desc { margin-top: 0.40rem; font-size: 1.00rem; }
...
# ===========================
# AWS Rekognition (Celebrity + face fallback)
# ===========================
@st.cache_resource
...
def build_api_queries(claim_en: str, nlp) -> List[str]:
    """
    Best-effort query generator for APIs.
    Goal: produce queries like "Virat Kohli retire ODI", "Virat Kohli ODI retirement" etc.
    """
    claim_en = normalize_text(claim_en)
    ...
    # 3) Entities + key verbs/nouns (spaCy)
    ents: List[str] = []
    ...
    if ents:
        ...
        # Combine top entity with a couple high-signal tokens
        # (this is the "Virat Kohli retire ODI" style)
        signal = []
        for t in base_tokens:
            if t in {"retire", "retired", "retirement", "odi", "test", "t20", "election", "blast", "explosion",
                ...
                signal.append(t)
        # fallback signal: first few tokens
        if not signal:
...
def render_landing():
    ...
    st.markdown(
        "A decision-support system for multilingual (Kannada/Hindi/Telugu/English) misinformation verification using evidence retrieval + NLI + visual cues."
    )
    st.markdown("---")
...
<div class="card">
  ...
  <div style="opacity:0.88;">
    ...
    • Runs Natural Language Inference (NLI) to judge claim alignment<br/>
    • Uses visual signals (CLIP, Deepfake CNN, AWS Rekognition) when an image is provided<br/>
    • Outputs an interpretable verdict: <b>SUPPORTED / CONTRADICTED / UNVERIFIED</b>
  </div>
</div>
""",
            unsafe_allow_html=True,
...
<div class="card" style="margin-top:12px;">
  <div class="smalllabel">Supported languages</div>
  <div>
    <span class="badge"><strong>English</strong></span>
    <span class="badge"><strong>Hindi</strong></span>
    <span class="badge"><strong>Kannada</strong></span>
    <span class="badge"><strong>Telugu</strong></span>
  </div>
</div>
""",
            unsafe_allow_html=True,
...
<div class="card">
  ...
  <div style="opacity:0.88; line-height:1.6;">
    <b>Member 1:</b> [Name, USN]<br/>
    <b>Member 2:</b> [Name, USN]<br/>
    <b>Member 3:</b> [Name, USN]<br/>
    <b>Member 4:</b> [Name, USN]<br/>
  </div>
</div>
""",
            unsafe_allow_html=True,
        ...
        st.markdown(
            """
<div class="card" style="margin-top:12px;">
  <div class="smalllabel">Guide / Mentor</div>
  <div style="opacity:0.88;">
    <b>[Mentor Name]</b><br/>
...
""",
    ...
    with b3:
        st.caption("Tip: Add a short headline and (optional) image on the next page to run the full pipeline.")
...
def render_system():
    ...
    with st.sidebar.expander("Run command", expanded=False):
        st.code("streamlit run app_streamlit_final_v5_multipage_best.py", language="bash")
    ...
    if not news_text or not news_text.strip():
        ui_callout("Enter a headline/claim in the sidebar to start. You can also upload an image (optional).")
        return
    ...
    with left:
        st.markdown("## Claim")
        st.markdown(
            f"<div class='smalllabel'>Original</div><div class='card'><b>{T_raw}</b></div>",
            unsafe_allow_html=True,
        )
        st.markdown(
    ...
    if show_debug:
        ...
        try:
            ...
            st.sidebar.write("st.secrets read error:", str(e))
    ...
    if image is not None:
        ...
        if deepfake_model is None:
            df_display = f"Unavailable: {deepfake_status}"
        else:
        ...
        if rek_client is None:
            rek_display = f"Unavailable: {rek_status}"
        else:
            ...
            if celeb_name is None:
                ...
                rek_display = f"{note} · Faces detected: {faces_n} ({faces_note})"
            else:
    ...
    with st.spinner("Fetching evidence (APIs + RSS)..."):
        # APIs (run multiple queries, dedup later)
        ...
        # RSS always (English edition)
        rss_any_ok, last_err = False, ""
    ...
    if not items:
        ui_callout(
            "<b>No evidence items were found.</b> "
            "This can happen for very local stories or when APIs have no English matches. "
            "Try a shorter claim or include one strong entity (person/place)."
        )
        if show_debug:
            st.markdown("## Debug")
    ...
    # ===========================
    # VERDICT IN THE "BLANK SPACE" UNDER CLAIM
    # ===========================
...
<div class="card" style="margin-top:12px;">
  ...
  <div style="font-size:1.25rem; font-weight:750;">{max_ent:.2f}</div>
</div>
""",
            unsafe_allow_html=True,
        )
...
<div class="card" style="margin-top:12px;">
  ...
  <div style="font-weight:700;">{language_hint}</div>
