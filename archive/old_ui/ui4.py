# app_streamlit_final_v6_pro_ui_voice_evidence_image.py
# V6: Pro UI refresh + evidence-image preview + voice-over (browser) + better query generation
#
# IMPORTANT: Core pipeline logic is preserved (same gating, NLI, CLIP, deepfake, rekognition flow).
# Changes are UI/UX + evidence-image enrichment + query generation improvements + explicit API-first then RSS fallback.
...
# PAGE SETUP
# ===========================
st.set_page_config(page_title="JaanKaar AI — Fake News Detection", layout="wide")
...
if "last_voice_payload" not in st.session_state:
    st.session_state.last_voice_payload = ""
...
h3 { font-size: 1.15rem !important; margin-top: 0.85rem !important; }
p, li, div, span { font-size: 0.99rem; line-height: 1.55; }

/* ---------- Hero ---------- */
.hero {
  border: 1px solid rgba(255,255,255,0.10);
  background: radial-gradient(1200px 600px at 10% 10%, rgba(99,102,241,0.22), rgba(255,255,255,0.02) 60%),
              radial-gradient(900px 500px at 90% 40%, rgba(14,165,233,0.15), rgba(255,255,255,0.02) 55%),
              rgba(255,255,255,0.02);
  border-radius: 22px;
  padding: 18px 18px;
}
.hero-title { font-size: 2.65rem; font-weight: 800; margin: 0; }
.hero-sub { opacity: 0.86; margin-top: 0.25rem; }
.pill {
...
.card-title { font-weight: 750; margin-bottom: 0.4rem; font-size: 1.05rem; }
.smalllabel { font-size: 0.82rem; opacity: 0.72; margin-bottom: 0.2rem; }
...
.verdict .v { font-size: 1.55rem; font-weight: 820; letter-spacing: 0.3px; margin: 0; }
...
.evidence .title { font-weight: 780; margin-bottom: 0.35rem; font-size: 1.06rem; }
.evidence .meta { opacity: 0.82; font-size: 0.92rem; }
.evidence .desc { margin-top: 0.45rem; font-size: 0.99rem; opacity: 0.92; }
.evidence-grid {
  display: grid;
...
.evidence-img img { width: 100%; height: auto; display:block; }
.evidence-img .ph {
  padding: 14px; opacity: 0.75; font-size: 0.9rem;
}
...
def pick_best_image_url(candidates: List[str]) -> str:
    # Prefer https, non-empty, and not tiny placeholders
    for u in candidates:
        u = ensure_https(u)
        if not u:
            continue
        if any(x in u.lower() for x in ["sprite", "logo", "icon", "favicon"]):
            continue
        return u
    return ""
...
# ===========================
# Evidence-image enrichment (OG/Twitter cards)
# ===========================
@st.cache_data(show_spinner=False, ttl=60 * 30)
def extract_og_image(article_url: str) -> str:
    """
    Lightweight OG-image extractor to show an image for evidence items.
    Does NOT affect your core logic; purely UI.
    """
    if not article_url:
        return ""
    html, err = safe_get(article_url, timeout=8)
    if err or not html:
        return ""
    # Look for <meta property="og:image" content="..."> or twitter:image
    patterns = [
    ...
    return pick_best_image_url(cands)
...
# ===========================
# BETTER QUERY GENERATION (API-first, then RSS)
# ===========================
def _extract_entities_for_queries(claim_en: str, nlp) -> Dict[str, List[str]]:
    out = {"PERSON": [], "ORG": [], "GPE": [], "LOC": [], "EVENT": []}
    if not nlp or not claim_en.strip():
        return out
    try:
        doc = nlp(claim_en)
        for ent in doc.ents:
            if ent.label_ in out:
                ...
                if t and t not in out[ent.label_]:
                    out[ent.label_].append(t)
    except Exception:
        pass
    return out
...
def extract_keywords_for_api(claim_en: str, nlp) -> List[str]:
    """
    API queries should be tighter and entity-driven to reduce noisy retrieval.
    Keeps your pipeline intact; only improves query strings.
    """
    claim_en = normalize_text(claim_en)
    out: List[str] = []

    # 1) Full sanitized claim (still useful)
    q_full = sanitize_api_query(claim_en, max_len=140)
    if q_full:
        out.append(q_full)

    # 2) Entity-focused query (top entities)
    ents = _extract_entities_for_queries(claim_en, nlp)
    ent_tokens = []
    for k in ["PERSON", "ORG", "EVENT", "GPE", "LOC"]:
        ent_tokens.extend(ents.get(k, []))
    ent_tokens = ent_tokens[:5]
    if ent_tokens:
        q_ent = sanitize_api_query(" ".join(ent_tokens), max_len=120)
        if q_ent and q_ent.lower() not in {x.lower() for x in out}:
            out.append(q_ent)

    # 3) Keyphrase query (first ~9 words)
    q_short = sanitize_api_query(" ".join(claim_en.split()[:9]), max_len=120)
    if q_short and q_short.lower() not in {x.lower() for x in out}:
        out.append(q_short)

    # 4) If there is a PERSON, add an extra query: PERSON + top keyword
    if ents.get("PERSON"):
        kw = []
        for t in keyword_set_en(claim_en):
            kw.append(t)
        kw = [k for k in kw if k not in _EN_STOP]
        top_kw = kw[0] if kw else ""
        person = ents["PERSON"][0]
        q_p = sanitize_api_query(" ".join([person, top_kw]).strip(), max_len=90)
        if q_p and q_p.lower() not in {x.lower() for x in out}:
            out.append(q_p)

    return out[:4]

def build_rss_queries(original_text: str, claim_en: str, nlp) -> List[str]:
    """
    RSS queries can be broader; include original, english, plus an entity-boost query.
    """
    ...
    for k in ["PERSON", "ORG", "EVENT", "GPE", "LOC"]:
        ent_tokens.extend(ents.get(k, []))
    if ent_tokens:
        queries.append(" ".join(ent_tokens[:5]))
...
class EvidenceItem:
    ...
    desc: str = ""
    published: str = ""
    image_url: str = ""   # NEW: for evidence preview (UI only)
    relevance: float = 0.0
    f_ent: float = 0.0
    f_con: float = 0.0
    f_neu: float = 0.0
...
def google_news_rss(query: str, hl: str = "en-IN", gl: str = "IN", ceid: str = "IN:en") -> Tuple[List[EvidenceItem], str]:
    ...
    for e in feed.entries[:RSS_PER_QUERY]:
        ...
        if title and link:
            # RSS usually doesn't carry images reliably, so we fill later via OG scraping (UI only).
...
def fetch_newsapi_org(query: str, api_key: str) -> Tuple[List[EvidenceItem], str]:
    ...
    for a in res.get("articles", [])[:20]:
        ...
        img = ensure_https(a.get("urlToImage", "") or "")
        if title and link:
            items.append(EvidenceItem(title=title, url=link, source="NewsAPI.org", desc=desc, image_url=img))
    return items, ""
...
def fetch_gnews(query: str, api_key: str) -> Tuple[List[EvidenceItem], str]:
    ...
    for a in res.get("articles", [])[:20]:
        ...
        link = a.get("url", "") or ""
        desc = strip_html_garbage(a.get("description", "") or "")
        img = ensure_https(a.get("image", "") or "")
        if title and link:
            items.append(EvidenceItem(title=title, url=link, source="GNews(en)", desc=desc, image_url=img))
    return items, ""
...
def fetch_newsdata(query: str, api_key: str) -> Tuple[List[EvidenceItem], str]:
    ...
    for a in res.get("results", [])[:20]:
        ...
        desc = strip_html_garbage(a.get("description", "") or "")
        img = ""
        # NewsData sometimes returns image_url or image_url list depending on plan/version
        if isinstance(a.get("image_url", ""), str):
            img = ensure_https(a.get("image_url", "") or "")
        elif isinstance(a.get("image_url", None), list) and a.get("image_url"):
            img = ensure_https(a.get("image_url")[0] or "")
        if title and link:
            items.append(EvidenceItem(title=title, url=link, source="NewsData.io", desc=desc, image_url=img))
    return items, ""
...
def fetch_eventregistry(query: str, api_key: str) -> Tuple[List[EvidenceItem], str]:
    ...
    for a in res.get("articles", {}).get("results", [])[:20]:
        ...
        # EventRegistry may include "image" sometimes (depends); attempt common keys.
        img = ensure_https(a.get("image", "") or a.get("imageUrl", "") or "")
        if title and link:
            items.append(EvidenceItem(title=title, url=link, source="EventRegistry(en)", desc=desc, image_url=img))
    return items, ""
...
def enrich_evidence_images(items: List[EvidenceItem], max_to_enrich: int = 14) -> None:
    """
    UI-only enrichment: if an item has no image_url, try OG scraping.
    Keep it limited so the app remains responsive.
    """
    # Sort likely-good candidates first: those with higher relevance will be enriched after ranking,
    # but here we just do a quick pass (later we re-check top evidence too).
    c = 0
    for it in items:
        if c >= max_to_enrich:
            break
...
choice = st.sidebar.radio("Go to", ["Landing", "Detection System"], index=0 if st.session_state.page == "Landing" else 1)
st.session_state.page = "Landing" if choice == "Landing" else "System"
st.sidebar.markdown("---")

st.sidebar.markdown("### Output Voice-over")
st.session_state.auto_voice = st.sidebar.toggle("Auto-read verdict", value=st.session_state.auto_voice)
st.sidebar.caption("Uses your browser's built-in voice (no server TTS).")
st.sidebar.markdown("---")


# ===========================
# VOICE OVER (BROWSER SPEECH SYNTHESIS)
# ===========================
def speak_text_browser(text: str, autoplay: bool = True, rate: float = 1.0, pitch: float = 1.0):
    # Escape for JS
    js_text = (text or "").replace("\\", "\\\\").replace("`", "\\`").replace("\n", " ")
    auto_flag = "true" if autoplay else "false"
    # A tiny component that triggers speechSynthesis
    st.components.v1.html(
        ...
        <script>
          (function() {{
            ...
            if (autoplay) {{
              // Give the UI a moment to render before speaking
...
def render_landing():
    st.markdown(
        """
<div class="hero">
  <div class="pill">🧠 Multimodal • 🌐 Multilingual • 🛡️ Decision-support</div>
  <p class="hero-title">AI-based Multimodal Multilingual Regional Fake News Detection</p>
  <div class="hero-sub"><b>JaanKaar AI</b> — evidence retrieval + NLI + visual verification signals</div>
</div>
""",
        unsafe_allow_html=True,
    )
    ...
    with c1:
        st.markdown(
            ...
            <div class="card">
              ...
              <div style="font-size:1.03rem; line-height:1.75; opacity:0.95;">
                <b>Kartik Shekhar</b><br>
                <b>KV Nihal Mouni</b><br>
                <b>Kartik Kaul</b><br>
                <b>Tanishi Jain</b><br>
                <b>Priyanka S</b>
              </div>
              <div style="margin-top:10px; opacity:0.78; font-size:0.93rem;">
                Built with retrieval, multilingual translation, NLI, CLIP alignment, deepfake & celebrity mismatch checks.
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div class="card">
              ...
              <ul style="margin: 0.25rem 0 0 1.1rem;">
                <li>Accepts claims in multiple languages (auto-detect + translate for retrieval/NLI)</li>
                <li>Fetches evidence <b>APIs first</b>, then <b>Google News RSS</b> as fallback</li>
                <li>Scores relevance via embeddings, then runs NLI (entails/contradicts/neutral)</li>
                <li>Runs image checks: CLIP alignment, deepfake CNN, Rekognition celebrity match</li>
              </ul>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with c2:
        st.markdown(
            """
            <div class="card">
              <div class="card-title">ℹ️ About the pipeline</div>
              <div style="opacity:0.92;">
                This is an end-to-end decision-support pipeline designed for regional and multilingual fake news detection.
                It cross-checks a claim against retrieved evidence and visual signals, then produces a final verdict using learned gating.
              </div>
              <div style="margin-top:10px; opacity:0.82;">
                <b>Note:</b> Outputs are based on retrieved headlines/snippets + NLI + learned gating; verify with full articles when needed.
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            """
            <div class="card">
              <div class="card-title">🎯 New UI features (added)</div>
              <ul style="margin: 0.25rem 0 0 1.1rem;">
                <li><b>Evidence image preview</b> (from API image fields or article OG image)</li>
                <li><b>Voice-over</b> of verdict & summary using browser speech synthesis</li>
                <li>Cleaner layout using tabs, cards, and a top “At a glance” section</li>
              </ul>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with b2:
        if st.button("➡️ Enter Detection System", use_container_width=True):
            st.session_state.page = "System"
            st.rerun()


# ===========================
# SYSTEM PAGE (CORE PIPELINE PRESERVED)
# ===========================
def render_system():
    # ---------- Header
    st.markdown(
        """
<div class="hero">
  <div class="pill">🔎 Evidence-first • 🧾 NLI verification • 🖼️ Visual checks</div>
  <p class="hero-title" style="font-size:2.35rem;">JaanKaar AI — Fake News Detection</p>
  <div class="hero-sub">Cross-check a claim against retrieved evidence and visual signals.</div>
</div>
""",
        unsafe_allow_html=True,
    )
    ...
    news_text = st.sidebar.text_area("Enter headline / claim (any language)", height=130)
    show_debug = st.sidebar.checkbox("Show debug details", value=False)

    with st.sidebar.expander("Run command", expanded=False):
        st.code("streamlit run app_streamlit_final_v6_pro_ui_voice_evidence_image.py", language="bash")
    ...
    if news_text and len(news_text.strip()) > 0:
        ...
        # Top layout: claim + uploaded image preview
        ...
        with left:
            ...
            st.markdown(
                f"""
<div class="card">
  <div class="smalllabel">Original</div>
  <div style="font-weight:780; font-size:1.08rem;">{T_raw}</div>
  <div style="margin-top:8px;">
    <span class="badge"><strong>Detected language:</strong> {lang}</span>
    <span class="badge"><strong>spaCy:</strong> {spacy_name}</span>
  </div>
</div>
<div class="card">
  <div class="smalllabel">English (for retrieval / NLI)</div>
  <div style="font-weight:780; font-size:1.06rem;">{claim_en}</div>
</div>
""",
                unsafe_allow_html=True,
            )
        ...
        # ---------- Debug: secrets presence (preserved)
        if show_debug:
        ...
        # ===========================
        # VISUAL SIGNALS (compute first) — preserved logic
        # ===========================
        g_clip = 0.0
        ...
        # ===========================
        # LANGUAGE STYLE HINT (TF-IDF) — preserved logic
        # ===========================
        p_real = None
        p_fake = None
        language_hint = "n/a"
        ...
        # ===========================
        # FETCH EVIDENCE (APIs FIRST, RSS SECOND) — logic preserved, ordering made explicit
        # ===========================
        keys = {}
        try:
            keys = {
        ...
        rss_queries = build_rss_queries(T_raw, claim_en, nlp)
        api_queries = extract_keywords_for_api(claim_en, nlp)

        source_status: Dict[str, str] = {}
        items: List[EvidenceItem] = []

        with st.spinner("Fetching evidence (APIs → RSS fallback)..."):
            # ---------- APIs FIRST
            ...
            # ---------- RSS SECOND (fallback / additional coverage)
            rss_any_ok = False
            last_err = ""
        ...
        if not items:
            st.markdown(
                ...
                "Try a shorter claim or include a key entity name (place/person).</p></div>",
                unsafe_allow_html=True,
            )
            if show_debug:
        ...
        # Enrich images for top candidates only (UI-only)
        enrich_evidence_images(ranked[: min(16, len(ranked))], max_to_enrich=14)
        ...
        # ===========================
        # EXPLANATION (preserved content, cleaner presentation)
        # ===========================
        expl: List[str] = []
        ...
        # PRO UI: Tabs
        # ===========================
        tab1, tab2, tab3 = st.tabs(["📌 Results", "🧾 Evidence", "🧠 Debug"])

        # ---------- Results tab
        with tab1:
            st.markdown("## At a glance")

            # Voice payload (verdict + short summary)
            voice_summary = (
                f"Final verdict: {decision}. "
                f"Evidence strength: {max_ent:.2f}. "
                f"CLIP alignment: {g_clip:.2f}. "
            )
            if mismatch_msg:
                voice_summary += "Warning: possible person mismatch between claim and image. "
            if rep_locs:
                voice_summary += "Consensus locations: " + ", ".join(rep_locs[:3]) + ". "

            st.session_state.last_voice_payload = voice_summary

            vcol, scol, ecol = st.columns([0.52, 1.05, 0.95], gap="large")
...
<div class="card" style="margin-top:12px;">
  ...
  <div style="font-size:1.35rem; font-weight:800;">{max_ent:.2f}</div>
</div>
""",
                    unsafe_allow_html=True,
                )
...
<div class="card" style="margin-top:12px;">
  ...
  <div style="font-weight:780;">{language_hint}</div>
...
""",
                ...
                st.markdown("<div class='card-title'>🔊 Voice-over</div>", unsafe_allow_html=True)
                b1, b2 = st.columns([1, 1], gap="small")
                with b1:
                    if st.button("Read verdict", use_container_width=True):
                        speak_text_browser(st.session_state.last_voice_payload, autoplay=True, rate=1.0, pitch=1.0)
                with b2:
                    if st.button("Read explanation", use_container_width=True):
                        speak_text_browser(" ".join(expl[:4]), autoplay=True, rate=1.0, pitch=1.0)
            ...
            with scol:
                ...
                st.markdown(
                    f"""
<div class="card">
  <div class="card-title">🖼️ Visual signals</div>
  <div><span class="badge"><strong>CLIP alignment:</strong> {g_clip:.3f} · {clip_note}</span></div>
  <div><span class="badge"><strong>Deepfake CNN:</strong> {df_display}</span></div>
  <div><span class="badge"><strong>AWS Rekognition:</strong> {rek_display}</span></div>
</div>
""",
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    f"""
<div class="card" style="margin-top:12px;">
  <div class="card-title">📈 Retrieval snapshot</div>
  <div><span class="badge"><strong>Evidence items:</strong> {len(ranked)}</span>
       <span class="badge"><strong>Max relevance:</strong> {max_rel:.2f}</span>
       <span class="badge"><strong>Top contradiction used:</strong> {max_con_used:.2f}</span>
  </div>
</div>
""",
                    unsafe_allow_html=True,
            ...
            with ecol:
                # Show top evidence image (new)
                top_item = scored[0] if scored else ranked[0]
                top_img = top_item.image_url
                if not top_img:
                    top_img = extract_og_image(top_item.url)  # try once more for the #1 item
                    if top_img:
                        top_item.image_url = top_img

                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='card-title'>📰 Top evidence preview</div>", unsafe_allow_html=True)
                if top_img:
                    st.image(top_img, use_container_width=True)
                else:
                    st.markdown("<div style='opacity:0.75;'>No image available for top evidence.</div>", unsafe_allow_html=True)
                st.markdown(f"<div style='margin-top:8px; font-weight:760;'>{top_item.title}</div>", unsafe_allow_html=True)
                st.markdown(f"<div style='opacity:0.78; margin-top:6px;'>Source: {top_item.source} · Relevance: {top_item.relevance:.3f}</div>", unsafe_allow_html=True)
                st.markdown(f"<div style='margin-top:10px;'><a href='{top_item.url}' target='_blank'>Open article</a></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
        ...
        with tab2:
            ...
            # Enrich images for the shown set (UI-only, bounded)
            enrich_evidence_images(shown, max_to_enrich=10)

            for it in shown:
                desc_line = (it.desc or "").strip()
                if len(desc_line) > 260:
                    desc_line = desc_line[:260] + "..."

                img_html = ""
                if it.image_url:
                    img_html = f"<div class='evidence-img'><img src='{it.image_url}'/></div>"
                else:
                    img_html = "<div class='evidence-img'><div class='ph'>No image found for this evidence</div></div>"
  ...
  <div class="evidence-grid">
    ...
    <div>
      ...
      <div class="meta">
        ...
        Contradiction used: {it.weighted_con_used:.2f}
      </div>
      <div class="desc">{desc_line}</div>
      <div style="margin-top:10px;"><a href="{it.url}" target="_blank">Read article</a></div>
    </div>
  </div>
</div>
""",
        ...
        # ---------- Debug tab (only show if user wants; otherwise show a short hint)
        with tab3:
            if not show_debug:
                st.markdown(
                    "<div class='card'>Enable <b>Show debug details</b> in the sidebar to view source status, queries, features, and gate outputs.</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown("## Debug")
                ...
                st.write(
                    {
                        ...
                        "contra_note": contra_note,
                        "support_thr": support_thr,
                        "contra_thr": contra_thr,
                    }
                )
                st.markdown("### Rekognition debug")
                st.write(rek_debug)

        st.markdown("---")
        st.caption(
            "Decision-support system only. Outputs are based on retrieved headlines/snippets, NLI, and learned gating; "
            "verify using full articles when needed."
        )

    else:
        st.markdown("<div class='card'>Enter a claim in the sidebar to begin.</div>", unsafe_allow_html=True)
