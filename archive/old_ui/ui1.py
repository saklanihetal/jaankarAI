...
# 1) Google News RSS ceid is language-correct (IN:kn / IN:hi / IN:te) -> avoids "OK but 0 items"
# 2) GNews/API query is sanitized to avoid HTTP 400 "query syntax error"
# 3) APIs run on SAFE English queries (claim_en + short) for stability; RSS still runs multilingual
# 4) Rekognition: if CelebrityFaces empty, we run detect_faces fallback and show face count
...
# IMPORTANT: correct Google News edition ids
...
def rekognition_detect_faces_count(pil_img: Image.Image, client) -> Tuple[int, str]:
    """
    Fallback diagnostic: are faces being detected at all?
    """
    try:
        buf = io.BytesIO()
        pil_img.convert("RGB").save(buf, format="JPEG", quality=92)
...
if news_text and len(news_text.strip()) > 0:
    ...
    if image is None:
        ...
        if rek_client is None:
            ...
            if celeb_name is None:
                st.warning(note)
                # fallback: are faces detectable at all?
                faces_n, faces_note = rekognition_detect_faces_count(image, rek_client)
                st.caption(f"Faces detected (fallback): {faces_n} ({faces_note})")
                if show_debug:
    ...
    # API queries (use stable English + short)
    api_queries = []
    ...
    with st.spinner("Fetching evidence (APIs + RSS)..."):
        # --------- APIs: run on safe English queries ----------
