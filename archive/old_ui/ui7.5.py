...
# V6 (Enhanced UX): Interactive Loading + Progressive Disclosure
# - Solves the "Blank Screen" problem using st.status and live placeholders
# - Displays generated queries immediately
# - Displays retrieved headlines while NLI is processing
# - Improved CSS for "Processing" state
...
def render_system():
    # Load resources immediately so the app is ready (cached)
    R = load_all_resources()
    
    # ... (Unpack R resources here for clarity) ...
    device = R["device"]
    ...
    if run_btn:
        with status_container:
            # Expandable status log - keeps user engaged
            with st.status("Initiating verification pipeline...", expanded=True) as status:
                ...
                # 2. GENERATE QUERIES (Show them immediately!)
                st.write(" Generating search queries from claim...")
                rss_queries = build_rss_queries(T_raw, claim_en)
                api_queries = build_api_queries(claim_en, nlp)
                
                # Display generated queries to user (Engagement hook 1)
