# app_streamlit_final_v4_fixed.py
# V4 (fixed): same pipeline + better AWS Rekognition diagnostics + APIs actually run & show counts
# Key fixes:
# 1) Rekognition is NOT shown as "disabled" unless keys are missing OR boto3 missing.
#    If AWS call fails (permissions/region/etc), we show the real error string.
# 2) News APIs run across ALL built queries (like RSS), not just claim_en.
# 3) Source status shows OK / error per source, and we show per-source item counts so you can verify APIs ran.
...
def load_rekognition_client():
    ...
    Returns (client_or_none, status_string, debug_flags)
    """
...
def rekognition_detect_celebrity_from_image(pil_img: Image.Image, client) -> Tuple[Optional[str], float, str]:
    ...
    try:
        ...
        # THIS is the real failure (permissions, invalid keys, wrong region, etc.)
...
def dedup_items(items: List[EvidenceItem]) -> List[EvidenceItem]:
    # Dedup by title+url to avoid killing cross-source evidence
    seen = set()
    out = []
    for it in items:
...
with st.sidebar.expander("Run command", expanded=False):
    st.code("streamlit run app_streamlit_final_v4_fixed.py", language="bash")
...
if news_text and len(news_text.strip()) > 0:
    ...
    if show_debug:
        # This confirms secrets are actually visible to Streamlit (without printing them).
    ...
    if image is None:
        ...
        if rek_client is None:
            # Only truly "disabled" if missing keys / boto3 / secrets read failed
            st.caption(f"Rekognition unavailable: {rek_status}")
            if show_debug:
                st.write("Rekognition debug:", rek_debug)
            st.caption("To enable: ensure .streamlit/secrets.toml is correct and restart Streamlit.")
        else:
            celeb_name, celeb_conf, note = rekognition_detect_celebrity_from_image(image, rek_client)
            if celeb_name is None:
                st.warning(note)  # shows real AWS error OR "no celebrity detected"
                if show_debug:
                    st.write("Rekognition status:", rek_status)
                    st.write("Rekognition debug:", rek_debug)
            else:
    ...
    with st.spinner("Fetching evidence (APIs + RSS)..."):
        # --------- APIs: run across ALL queries so they actually contribute ----------
    ...
    # Show per-source counts so you can confirm APIs actually ran
    counts: Dict[str, int] = {}
    for it in items:
