...
# Final integrated Streamlit app (patched + Deepfake + AWS Rekognition):
# - Keeps "Consensus context found in evidence" (non-negotiable)
# - RSS + optional News APIs, multilingual -> English (NLLB), relevance (LaBSE), NLI (BART MNLI)
...
if news_text and len(news_text.strip()) > 0:
    ...
    with st.spinner("Fetching evidence..."):
        ...
        # Language-biased RSS (helps regional indexing)
        if lang in {"hi", "kn", "te"}:
