# app_streamlit_final_v5_landing_and_ui.py
# V5: Landing page + System page (full pipeline preserved)
# Changes:
# 1) Added Landing page (Project title + Team details + About + Enter button)
# 2) System page = your full V4 UI polished pipeline (preserved)
# 3) Title bigger + Evidence section text slightly bigger (CSS only)
...
# ===========================
# GLOBAL STYLES (UPDATED: bigger title + slightly bigger evidence)
# ===========================
st.markdown(
    """
<style>
...
/* Keep others consistent */
h2 { font-size: 1.35rem !important; margin-top: 1.25rem !important; }
h3 { font-size: 1.10rem !important; margin-top: 1.0rem !important; }
...
.badge {
  ...
  padding: 4px 10px;
  border-radius: 999px;
  font-size: 0.86rem; /* slightly bigger */
...
/* Evidence items (UPDATED bigger) */
.evidence {
  border: 1px solid rgba(255,255,255,0.08);
  background: rgba(255,255,255,0.02);
  border-radius: 14px;
  padding: 14px 14px; /* slightly bigger padding */
  margin-bottom: 12px;
}
.evidence .title { font-weight: 700; margin-bottom: 0.35rem; font-size: 1.05rem; } /* bigger title */
.evidence .meta { opacity: 0.80; font-size: 0.92rem; } /* bigger meta */
.evidence .desc { margin-top: 0.45rem; font-size: 1.00rem; } /* bigger desc */
...
# ===========================
def render_landing():
    st.markdown("# AI-based Multimodal Multilingual Regional Fake News Detection")
    st.markdown("**JaanKaar AI**")
    st.markdown("---")
    ...
    with c2:
        st.markdown(
            """
            <div class="card">
              <h2 style="margin-top:0;">ℹ️ About the System</h2>
              <div style="font-size:1.03rem; line-height:1.65;">
                This system detects and verifies fake news using a <b>multimodal</b> and <b>multilingual</b> AI pipeline.
                It analyzes textual claims, retrieves evidence from news sources (APIs + RSS), performs
                <b>Natural Language Inference (NLI)</b>, image–text consistency checks (CLIP),
                deepfake detection, and celebrity mismatch detection.
                <br><br>
                It is an end-to-end decision-support system designed for regional and multilingual fake news detection.
              </div>
            </div>
            """,
            unsafe_allow_html=True,
...
# ===========================
# SYSTEM PAGE (YOUR ORIGINAL PIPELINE UI)
# ===========================
def render_system():
    # ===========================
    # HEADER
    ...
    with st.sidebar.expander("Run command", expanded=False):
        st.code("streamlit run app_streamlit_final_v5_landing_and_ui.py", language="bash")
    ...
    if news_text and len(news_text.strip()) > 0:
        ...
        # ===========================
        # VERDICT + SIGNALS (same as your UI)
        # ===========================
